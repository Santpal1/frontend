...
const FacultyDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFacultyDetails = async () => {
      ...
      try {
        ...
        const response = await axios.get(`https://srm-sp-production-dc63.up.railway.app/api/faculty/${id}`, {
          params: {
        ...
        if (!criteriaStartFilter && !criteriaEndFilter) {
          try {
            const quartRes = await axios.get(`https://srm-sp-production-dc63.up.railway.app/api/faculty/${id}/quartile-summary`, { headers: getAuthHeaders() });
  ...
  useEffect(() => {
    const fetchTypeCounts = async () => {
      ...
      try {
        const response = await axios.get(`https://srm-sp-production-dc63.up.railway.app/api/faculty/${id}/type-count`, {
          params: {
            sdg: sdgFilter !== "none" ? sdgFilter : undefined,
            domain: domainFilter !== "none" ? domainFilter : undefined,
            year: yearFilter !== "none" ? yearFilter : undefined,
  ...
  useEffect(() => {
    const fetchCountryStats = async () => {
      if (!id) return;
      try {
        const res = await axios.get(`https://srm-sp-production-dc63.up.railway.app/api/faculty/${id}/country-stats`, { headers: getAuthHeaders() });
  ...
  useEffect(() => {
    const renderChart = async () => {
      ...
      try {
        ...
        let res;
        try { res = await axios.get(`https://srm-sp-production-dc63.up.railway.app/api/faculty/${id}/author-performance`, fullHistory ? { params: { full: 'true' } } : {}); }
        catch { res = await axios.get(`https://srm-sp-production-dc63.up.railway.app/api/faculty/author-performance/${id}`, fullHistory ? { params: { full: 'true' } } : {}); }
