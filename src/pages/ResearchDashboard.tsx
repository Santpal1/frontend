...
export default function ResearchDashboard() {
  ...
  const fetchTopAuthors = async (selectedTimeframe: string) => {
    try {
      ...
      let url = `https://srm-sp-production-dc63.up.railway.app/api/top-author?timeframe=${selectedTimeframe}`;
  ...
  const fetchData = async (selectedTimeframe: string) => {
    try {
      const headers = getAuthHeaders();
      let url = `https://srm-sp-production-dc63.up.railway.app/api/publications?timeframe=${selectedTimeframe}`;
      if (isAdmin() && departmentFilter && departmentFilter !== 'all') {
        url += `&department=${encodeURIComponent(departmentFilter)}`;
      }
      const res = await axios.get(url, { headers });
  ...
  const fetchQuartiles = async (selectedYear: string) => {
    try {
      ...
      const url =
        selectedYear === "all"
          ? `https://srm-sp-production-dc63.up.railway.app/api/quartile-stats${isAdmin() && departmentFilter && departmentFilter !== 'all' ? `?department=${encodeURIComponent(departmentFilter)}` : ''}`
          : `https://srm-sp-production-dc63.up.railway.app/api/quartile-stats?year=${selectedYear}${isAdmin() && departmentFilter && departmentFilter !== 'all' ? `&department=${encodeURIComponent(departmentFilter)}` : ''}`;
      const res = await axios.get(url, { headers });
