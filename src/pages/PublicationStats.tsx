...
export default function PublicationStats() {
    ...
    const fetchPublicationStats = async (type: string, year: string) => {
        ...
        try {
            ...
            const res = await axios.get(
                `https://srm-sp-production-dc63.up.railway.app/api/publication-stats?${params.toString()}`,
                { headers }
            );
    ...
    const fetchPapersForPublication = async (publicationName: string) => {
        ...
        try {
            ...
            const headers = getAuthHeaders();
            const res = await axios.get(
                `https://srm-sp-production-dc63.up.railway.app/api/publication-papers?${params.toString()}`,
                { headers }
            );
            setPapersModal((prev) => ({
                ...prev,
