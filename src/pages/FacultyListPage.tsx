...
const FacultyListPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchAvailableDomains = async () => {
      try {
        ...
        const url = `http://localhost:5001/api/faculty/available-domains?department=${encodeURIComponent(departmentFilter)}`;
  ...
  const fetchFaculty = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:5001/api/faculty", { params, headers });
  ...
  const fetchCriteriaFilteredFaculty = async () => {
    ...
    try {
      ...
      const headers = getAuthHeaders();
      if (isAdmin() && departmentFilter && departmentFilter !== "all") params.department = departmentFilter;
      const response = await axios.get("http://localhost:5001/api/faculty/criteria-filter", { params, headers });
  ...
  const fetchFacultyByTimeframe = async (selectedTimeframe: string) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:5001/api/faculty/papers?timeframe=${selectedTimeframe}`,
        { headers, params }
      );
  ...
  const downloadPapersIEEE = async () => {
    ...
    try {
      ...
      const headers  = getAuthHeaders();
      const response = await axios.get(
        `http://localhost:5001/api/faculty/papers-ieee/export?${params.toString()}`,
        { headers }
      );
                        ...
                        onClick={async () => {
                          try {
                            ...
                            const res = await axios.get(
                              `http://localhost:5001/api/faculty/${member.faculty_id}`,
                              { headers: h }
                            );
                            const ids: string[] =
