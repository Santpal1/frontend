...
const AdvancedSearch: React.FC = () => {
  ...
  const fetchFilterOptions = async () => {
    try {
      ...
      const [sdgRes, domainRes] = await Promise.all([
        axios.get('https://srm-sp-production-dc63.up.railway.app/api/sdg/list', { headers }),
        axios.get('https://srm-sp-production-dc63.up.railway.app/api/insights/domains', { headers }),
      ]);
      setSDGs(sdgRes.data || []);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const headers = getAuthHeaders();
      const response = await axios.get(`https://srm-sp-production-dc63.up.railway.app/api/search/advanced?${params.toString()}`, { headers });
