...
const ImpactAnalytics: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let url = `https://srm-sp-production-dc63.up.railway.app/api/impact-analytics`;
  ...
  const fetchDepartments = async () => {
    try {
      const headers = getAuthHeaders();
      const res = await axios.get('https://srm-sp-production-dc63.up.railway.app/api/impact-analytics/departments', { headers });
  ...
  const fetchAdvanced = async () => {
    try {
      const headers = getAuthHeaders();
      let url = `https://srm-sp-production-dc63.up.railway.app/api/impact-analytics/advanced`;
      if (isAdmin && isAdmin() && dept !== 'all') url += `?department=${encodeURIComponent(dept)}`;
      const res = await axios.get(url, { headers });
